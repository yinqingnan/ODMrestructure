            ...
            >
              ...
              >
                ...
                <span
                  class="iconfont"
                  :class="item.icon"
                  style="margin-left:4px;font-size: 17px;margin-right: 10px;"
                ></span>
                <span>{{ item.name }}</span>
              </router-link>
            </a-menu-item>
...
export default class LeftMenu extends Vue {
  ...
  private mounted() {
    ...
    if (agent.indexOf("chrome") > 0) {
      if (
        Number(agent.match(regStr_chrome)[0].split("/")[1].split(".")[0]) > 50
      ) {
  ...
  private getdata() {
    this.list = JSON.parse(localStorage.getItem("navlist"))
    this.findIndexArray(this.list, this.$route.name, [])[0]
  ...
  routechange(to: any) {
    ...
    this.$set(this.defaultSelectedKeys, 0, to.meta.key)
